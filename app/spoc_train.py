...
caffe_root='/home1/landlee/BAK/python27/caffe'
...
def thread_train_spoc(tid, file_list, dlist, out_base):
    print "Task [%2d] running, dir range: %d ~ %d" %(tid, file_list[0], file_list[1])
    
    model = caffe_model(model_id=0, model_init=0)
    dir_cnt = 0
    for dir in dlist:
        dir_cnt += 1       
        ...
        print("  --- [%2d] Spoc feat: %d: %s   frame: %d" % (tid, dir_cnt, dir, len(flist)))
        out_dir = os.path.join(out_base, dir.split('/')[-1]) + '.mp4'
        if not os.path.exists(out_dir): os.makedirs(out_dir)
        i=0
        for f in flist:
            ...
            frame_id = get_frame_id(src)
            dat_name = src.split('/')[-2]+'.mp4_frame_id['+str(frame_id)+']_image_sn['+str(i)+']'+'.dat'
            save_npy = os.path.join(out_dir, dat_name)
            save_vlad(save_npy, spoc, 512)              
            i+=1
            #print "save_npy=",save_npy;exit()
...
def extract_refer_spoc(thread_num=1):
    ...
    for tid in range(thread_num):
        ...
        task = pop.task(func=thread_train_spoc, args=(tid+1, (start,end), dlist, out_base))
        task.run()
...
if __name__ =='__main__':
    extract_refer_spoc(thread_num=20)
